...
if ! command -v rendercv >/dev/null 2>&1; then
  echo "rendercv not found. Install with: python3 -m pip install 'rendercv==2.8'" >&2
  exit 1
fi

rm -rf rendercv_output/en rendercv_output/ja
mkdir -p rendercv_output/en rendercv_output/ja
mkdir -p public/en public/ja public/downloads

rendercv render cv/Yuan_Zhang_EN_CV.yaml
rendercv render cv/Yuan_Zhang_JA_CV.yaml

EN_HTML="$(find rendercv_output/en -maxdepth 1 -name '*.html' | head -n 1)"
JA_HTML="$(find rendercv_output/ja -maxdepth 1 -name '*.html' | head -n 1)"
EN_PDF="$(find rendercv_output/en -maxdepth 1 -name '*.pdf' | head -n 1)"
JA_PDF="$(find rendercv_output/ja -maxdepth 1 -name '*.pdf' | head -n 1)"

if [[ -z "${EN_HTML}" || -z "${JA_HTML}" || -z "${EN_PDF}" || -z "${JA_PDF}" ]]; then
  echo "Missing RenderCV artifacts under rendercv_output/" >&2
  ls -R rendercv_output >&2 || true
  exit 1
fi

cp "${EN_HTML}" public/en/index.html
cp "${JA_HTML}" public/ja/index.html
